...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        logits = logits.view(input_ids.shape[0], input_ids.shape[1], -1)
        logits = logits + (lora.lm_head_lora(x_norm) if lora else 0)
        if lora:
            return F.cross_entropy(
                logits.float().reshape(-1, logits.size(-1)), targets, reduction="none"
...
def _accumulate_bpb(
    ...
    """Add one doc-chunk's contribution to running BPB accumulators."""
...
def eval_val_sliding_ttt(
    ...
    """Combined sliding window + LoRA TTT evaluation.

    Per-document processing:
      Phase 1 (Score): sliding window with LoRA for max-context scoring
      Phase 2 (Train): single forward pass over chunk for LoRA gradient update
    """
    files = sorted(glob.glob(args.val_files))
    all_tokens = torch.cat([load_data_shard(Path(f)) for f in files])
    docs = _find_docs(all_tokens)
    ...
    for doc_idx, (doc_start, doc_len) in enumerate(rank_docs):
        ...
        if pred_len < 1:
            continue
        doc_tokens = all_tokens[doc_start : doc_start + doc_len]

        # Fresh LoRA per document (batch_size=1)
        lora = BatchedTTTLoRA(1, base_model, lora_rank).to(device)
        opt = _build_ttt_optimizer(lora, args)
        ...
        for ci in range(num_chunks):
            ...
            # --- PHASE 1: Score chunk tokens using sliding windows ---
            # Slide through positions that cover [chunk_start, chunk_end) with
            # overlapping windows, scoring only the "new" stride-sized tail of each.
            p = chunk_start
            first_window = True
            while p < chunk_end:
                win_end = min(p + seq_len, pred_len)
                win_start = max(0, win_end - seq_len)

                # Which positions in this window to score
                if first_window and p == 0:
                    # Very first window of the document: score all positions
                    score_start_abs = chunk_start
                    first_window = False
                else:
                    # Score only the last `stride` positions (they're new)
                    score_start_abs = max(chunk_start, win_end - stride)
                    first_window = False
                score_end_abs = min(chunk_end, win_end)

                if score_start_abs >= score_end_abs:
                    p += stride
                    continue

                # Build input tensors
                toks = doc_tokens[win_start : win_end + 1].to(dtype=torch.int64, device=device)
                x = toks[:-1].unsqueeze(0)  # (1, win_len)
                y = toks[1:].unsqueeze(0)

                with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    logits = base_model.forward_logits(x, lora=lora)

                # Score the target positions
                s_off = score_start_abs - win_start  # offset within window
                s_len = score_end_abs - score_start_abs
                scored_logits = logits[0, s_off : s_off + s_len]
                scored_targets = y[0, s_off : s_off + s_len]

                per_tok_loss = F.cross_entropy(scored_logits.float(), scored_targets, reduction="none")
                loss_sum += per_tok_loss.to(torch.float64).sum()
                token_count += s_len

                # Byte counting
                prev_ids = x[0, s_off : s_off + s_len]
                tgt_ids = y[0, s_off : s_off + s_len]
                tb = base_bytes_lut[tgt_ids].to(torch.float64)
                tb += has_leading_space_lut[tgt_ids] & ~is_boundary_token_lut[prev_ids]
                byte_sum += tb.sum()

                p += stride

            # --- PHASE 2: Train LoRA on this chunk (if not last) ---
            if ci < num_chunks - 1:
                ws, wl, co, cl = _compute_chunk_window(ci, pred_len, num_chunks, chunk_size, seq_len)
                toks = doc_tokens[ws : ws + wl + 1].to(dtype=torch.int64, device=device)
                x_train = toks[:-1].unsqueeze(0)
                y_train = toks[1:].unsqueeze(0)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ptl = base_model(x_train, y_train, lora=lora)
                train_loss = ptl[0, co : co + cl].mean()
                opt.zero_grad()
                train_loss.backward()
                opt.step()

        if doc_idx % 500 == 0 and doc_idx > 0:
            log0 = lambda s: print(s, flush=True) if rank == 0 else None
            if log0:
                print(f"  sliding_ttt progress: {doc_idx}/{len(rank_docs)} docs", flush=True)
...
def main() -> None:
    ...
    # Combined sliding window + LoRA TTT evaluation (the best-of-both-worlds score)
    eval_stride = args.eval_stride if args.eval_stride > 0 else 64
    torch.cuda.synchronize()
    t_combined = time.perf_counter()
    combined_loss, combined_bpb = eval_val_sliding_ttt(
        args, base_model, rank, world_size, device,
        base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        stride=eval_stride,
    )
    torch.cuda.synchronize()
    log0(
        f"final_sliding_ttt val_loss:{combined_loss:.4f} val_bpb:{combined_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_combined):.0f}ms"
    )
    log0(f"final_sliding_ttt_exact val_loss:{combined_loss:.8f} val_bpb:{combined_bpb:.8f}")
