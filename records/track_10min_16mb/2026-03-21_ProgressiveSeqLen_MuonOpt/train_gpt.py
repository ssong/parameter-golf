...
class Hyperparameters:
    ...
    # Progressive sequence length: start short (fast steps), ramp to full length
    train_seq_len_start = int(os.environ.get("TRAIN_SEQ_LEN_START", 512))
    # Fraction of wallclock at which we reach full seq_len (0.5 = halfway)
    seq_len_ramp_frac = float(os.environ.get("SEQ_LEN_RAMP_FRAC", 0.5))
...
def main() -> None:
    ...
    if args.warmup_steps > 0:
        ...
        model.train()
        # Warmup at all progressive seq lengths to pre-compile torch.compile graphs
        warmup_seq_lens = sorted(set([args.train_seq_len_start, args.train_seq_len]))
        # Add intermediate powers of 2
        sl = args.train_seq_len_start
        while sl < args.train_seq_len:
            warmup_seq_lens.append(sl)
            sl *= 2
        warmup_seq_lens = sorted(set(warmup_seq_lens))
        steps_per_len = max(args.warmup_steps // len(warmup_seq_lens), 2)
        for wsl in warmup_seq_lens:
            for warmup_step in range(steps_per_len):
                zero_grad_all()
                for micro_step in range(grad_accum_steps):
                    if distributed:
                        model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
                    x, y = train_loader.next_batch(args.train_batch_tokens, wsl, grad_accum_steps)
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                ...
                for opt in optimizers:
                    opt.step()
                zero_grad_all()
            log0(f"warmup: compiled seq_len={wsl} ({steps_per_len} steps)")
    ...
    while True:
        ...
        # Progressive sequence length: ramp from start to full over first ramp_frac of training
        if max_wallclock_ms is not None and args.train_seq_len_start < args.train_seq_len:
            ramp_ms = max_wallclock_ms * args.seq_len_ramp_frac
            t = min(elapsed_ms / max(ramp_ms, 1.0), 1.0)
            # Step through powers of 2: 512 → 1024 → 2048
            log_start = math.log2(args.train_seq_len_start)
            log_end = math.log2(args.train_seq_len)
            cur_seq_len = int(2 ** (log_start + t * (log_end - log_start)))
            # Round to nearest power of 2 for efficiency
            cur_seq_len = 1 << int(round(math.log2(cur_seq_len)))
            cur_seq_len = min(cur_seq_len, args.train_seq_len)
        else:
